          ...
          <v-card class="h-100">
            ...
            <v-card-text class="pa-0">
              <v-list v-if="data.overdue.length" density="compact">
                <v-list-item v-for="task in data.overdue" :key="task.id" @click="$emit('open-task', task.id)">
                  <v-list-item-title>{{ task.title }}</v-list-item-title>
                  <v-list-item-subtitle>echeance {{ task.due_date }}</v-list-item-subtitle>
                </v-list-item>
              </v-list>
          ...
          <v-card class="h-100">
            ...
            <v-card-text class="pa-0">
              <v-list v-if="data.upcoming.length" density="compact">
                <v-list-item v-for="task in data.upcoming" :key="task.id" @click="$emit('open-task', task.id)">
                  <v-list-item-title>{{ task.title }}</v-list-item-title>
                  <v-list-item-subtitle>echeance {{ task.due_date }}</v-list-item-subtitle>
                </v-list-item>
              </v-list>
...
async function load() {
  loading.value = true;
  try {
    const { data: response } = await api.get("/dashboard/", { params: { project: props.project.id } });
    data.value = response;
  } finally {
    loading.value = false;
  }
